class TemperaturePart < Part

  attr_accessible :temperature_items_attributes

  has_many :temperature_items

  accepts_nested_attributes_for :temperature_items, allow_destroy: true
  ...
  def content
    ...
    temperature_items.each do |item|
      items << {
        title: item.title,
        value: item.value,
        url: item.url
