...
class ServiceController < ApplicationController
  ...
  private
    def generated_path
      ...
      ActiveSupport::Inflector.transliterate("/#{node.path.map{|a| a.title}.join('/')}/#{additional}/#{additional}.xhtml").downcase.gsub(/\s/,'_')
    end

    def decode_hash_to_path
      result = ''
      result << ActiveSupport::Inflector.transliterate("/#{Node.root}").downcase.gsub(/\s/,'_')
